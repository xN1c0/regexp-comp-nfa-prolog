 ...
 *and calls an auxiliar procedure
 */
is_regexp(RE) :-
    nonvar(RE),
    is_regexp_helper(RE),
...
/*
 * calls a second auxiliar procedure
 */
is_regexp_helper(RE) :-
    RE =.. Rs,
...
/*
 * procedure for the '/' (OR) functor
 * calls an auxiliar procedure for the recursion
 */
regexp_split_case(['/'|Rs]) :-
...
/*
 * procedure for the '[]' (SEQUENCE) functor
 * calls an auxiliar procedure for the recursion
 */
regexp_split_case(['[|]'|Rs]) :-
    is_regexp_check_all(Rs),
 ...
 * calls a second procedure to create the nfa
...
/*
 * general case
 * calls recursion unifying the functor of the list in the list
 */
nfa_regexp_comp_base(FA_Id, Base, S, F) :-
    Base =.. Sub_base,
